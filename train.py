...
def main(args):
    ...
    #Если нужно обучить или дообучить модель никак не изменяя загружаемые веса
    state_dict = find_model('best.pt')
    ...
    for epoch in range(args.epochs):
        ...
        for latent in loader:
            ...
            latent = latent.squeeze(dim=1)
            t = torch.randint(0, diffusion.num_timesteps, (latent.shape[0],), device=device)
            loss_dict = diffusion.training_losses(model, latent, t)
